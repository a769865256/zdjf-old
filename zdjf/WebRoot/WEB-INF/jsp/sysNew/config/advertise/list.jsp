...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 配置管理 <span class="c-gray en">&gt;</span> 广告位管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray">
		<span class="l">
			<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
			<a class="btn btn-primary radius" href="javascript:;" onclick="admin_role_add('添加链接','<%=path%>/config/advertise/toDetail.action?advertiseType=1','800','550')"><i class="Hui-iconfont">&#xe600;</i> 新增广告位</a>
		</span>
		<span class="r">共有数据：<strong>${detail.total }</strong> 条</span> </div>
	<table class="table table-border table-bordered table-hover table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="10">广告位管理</th>
			</tr>
			<tr class="text-c">
				<th width="150">ID</th>
				<th width="150">标题</th>
		...
		<tbody>
			<c:forEach var="line"  items="${detail.dataList }">
                            <tr  class="trLine" >
                                ...
                                <td class="am-text-middle">${line.image_url }</td>
                                <td class="am-text-middle">${line.view_count }</td>
                                ...
                                <td class="am-text-middle">
                                 <c:if test="${line.position==1 }">
                                首页
                                </c:if>
                                </td>
                                <td class="am-text-middle">${line.order_number }</td>
                                <td class="am-text-middle">${line.is_show }</td>
                                <td class="am-text-middle"><fmt:formatDate value="${line.update_time }" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                                <td class="am-text-middle ad_update"><a onclick="admin_role_add('添加链接','<%=path%>/config/advertise/toDetail.action?advertiseType=2&advertiseId='+${line.id },'800','550')">编辑</a>
                           		<a href="javascript:;" data-id="${line.id }"  class="delete"> 删除</a></td>
                            </tr>
                        </c:forEach>
		</tbody>
	</table>
...
$('.delete').click(function(){
	...
	var url = '<%=path%>' + "/config/advertise/delete.action";
	$.post(url, {"advertise_id" : advertise_id}, function(data) {
				alert("删除成功");
				url = '<%=path%>' + "/config/advertise/toList.action";
				window.location = url;
			});
});
/*管理员-角色-添加*/
		...
		layui.use(['layer','laypage','element','carousel'], function(){
			...
			laypage.render({  /*参考链接http://www.layui.com/doc/modules/laypage.html*/
				...
				jump:function(obj,first){
					if(!first){
						window.location=path+'/config/advertise/toList.action?currentPage='+obj.curr+'&limit=10';
					}
				}
			});
		})

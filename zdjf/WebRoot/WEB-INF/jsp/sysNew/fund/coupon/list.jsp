...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			<a href="javascript:;" onclick="picture_add('新增加息券','<%=path %>/sys/fund/coupon/toDetail.action?giftType=1')" class="btn btn-primary radius"> 新增加息券</a>
		</span>
		...
		<tbody>
				<c:forEach var="coupon"  items="${detail.dataList }">
						...
						<td class="am-text-middle">
                            ...
                            <td>
                            	<c:forEach var="ct"  items="${params.couponType }">
                            		<c:if test="${ct.data_field_value==coupon.coupon_source }">${ct.data_field_name }</c:if>
                            	</c:forEach>
                            </td>
                            <td><fmt:formatNumber value="${coupon.interest }" type="number" pattern="0.00"/>%</td>
                            <td>
                            	${coupon.invalid_value }
                            </td>
                            ...
                            <td>
                                <div class="am-btn-toolbar">
                                    <div class="am-btn-group am-btn-group-xs">
                                           <a href="javascript:;" onclick="picture_add('编辑加息券','<%=path %>/sys/fund/coupon/toDetail.action?giftType=2&giftId=${coupon.id }')" class="btn btn-primary radius"> 编辑</a>
											<a href="javascript:;" data-id="${coupon.id }"  class="btn btn-primary delete radius"> 删除</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
...
$('.delete').click(function(){
	...
	var url = '<%=path%>' + "/sys/fund/coupon/delete.action";
	$.post(url, {"gift_id" : gift_id}, function(data) {
				alert("删除成功");
				url = '<%=path%>' + "/sys/fund/coupon/toList.action";
				window.location = url;
			});
});
var path='<%=path%>';
		...
		layui.use(['layer','laypage','element','carousel'], function(){
			...
			laypage.render({ 
				...
				jump:function(obj,first){
					if(!first){
						window.location=path+'/sys/fund/coupon/toList.action?currentPage='+obj.curr+'&limit=10';
					}
				}
			});
		});

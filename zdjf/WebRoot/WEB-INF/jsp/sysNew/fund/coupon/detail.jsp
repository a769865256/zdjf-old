...
<title>优惠管理 - 加息券列表 - <c:if
		test="${null == giftType || giftType == ''|| giftType==2}">添加加息券</c:if> <c:if
		test="${null != giftType && giftType ==2}">编辑加息券</c:if></title>
<meta name="keywords"
...
<body>
	<article class="page-container">
		<form class="form form-horizontal" id="form-article-add"
			action="<%=path%>/sys/fund/coupon/add.action" method="post">
			<input type="hidden" name="giftId" value="${giftId }">
			<input type="hidden" name="giftType" value="${giftType }">
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2"><span
					class="c-red">*</span>加息券名称：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"
						value="<c:if test="${null != detail }">${detail.coupon_name}</c:if>"
						placeholder="输入加息券名称" id="gift_name" name="gift_name">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2"><span
					class="c-red">*</span>加息券面值(%)：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"
						value="<c:if test="${null != detail }">${detail.interest}%</c:if>"
						placeholder="输入加息券金额" id="money" name="money">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2"><span
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"
						value="<c:if test="${null != detail }">${detail.min_days}</c:if>"
							...
							<c:if test="${gift_source != null }">
								<c:forEach var="item" items="${gift_source }">
									<option value="${item.data_field_value }"
										<c:if test="${null != detail && detail.coupon_source == item.data_field_value }">selected</c:if>>${item.data_field_name }</option>
								</c:forEach>
							</c:if>
					</select>
					</span>
						...
						data-am-selected="{btnWidth: '100%', btnStyle: 'secondary'}">
							...
							<c:if test="${date_type != null }">
								<c:forEach var="item" items="${date_type }">
									<option value="${item.data_field_value }"
										<c:if test="${null != detail && detail.invalid_type == item.data_field_value }">selected</c:if>>${item.data_field_name }</option>
								</c:forEach>
							</c:if>
					</select>
					</span>
			...
			<div class="row cl">
				...
				<div class="am-u-sm-4">
					<div class="am-btn-group am-u-sm-12 no-padding" data-am-button>
						<label
							class="am-btn am-btn-secondary am-u-sm-4 diff_mode_day dateType  <c:if test="${detail.invalid_type == 2 || detail==null}">am-active</c:if>"
							style="float: left;"> <input type="radio"
							id="diff_mode_day" name="date_type" value="2"
							<c:if test="${detail.invalid_type == 2 || detail==null}">checked="checked"</c:if>>
							固定天数
						</label>
						<label
							class="am-btn am-btn-secondary am-u-sm-4 diff_mode_date dateType">
							<input type="radio" id="diff_mode_date" name="date_type"
							value="1" <c:if test="${detail.invalid_type == 1 }">checked</c:if>>
							指定日期
						</label> 
				...
				<div class="am-u-sm-5 giftDays <c:if test="${detail.invalid_type == 1 }">hide</c:if>">
					<input type="number" id="gift_days" name="gift_days"
						value="${detail.invalid_days }" placeholder="输入大于0的数字" />
				</div>
			</div>
			
			<div class="row cl startDate <c:if test="${detail.invalid_type == 2 || detail==null}">hide</c:if>">
					<label class="form-label col-xs-4 col-sm-2"><span
					...
					<input type="text"
						...
						style="width:120px;" value="<c:if test="${null != detail }"><fmt:formatDate value="${detail.invalid_start_date }" type="both" pattern="yyyy-MM-dd" /></c:if>">
					</div>
			</div>
			<div class="row cl endDate <c:if test="${detail.invalid_type == 2 || detail==null}">hide</c:if>">
				<label class="form-label col-xs-4 col-sm-2"><span
					class="c-red">*</span>加息券结束日期：</label>
					<div class="formControls col-xs-8 col-sm-9">
					<input type="text"
						onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'start_date\')}' })"
						id="end_date" name="end_date" class="input-text Wdate"     
						style="width:120px;" value="<c:if test="${null != detail }"><fmt:formatDate value="${detail.invalid_end_date }" type="both" pattern="yyyy-MM-dd" /></c:if>">
					</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2"><span
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"
						value="<c:if test="${null != detail }">${detail.limit_count}</c:if>"

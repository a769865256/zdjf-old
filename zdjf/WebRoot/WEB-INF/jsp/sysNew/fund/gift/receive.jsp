	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
				<c:forEach var="line"  items="${detail.dataList }">
								<tr>
									<td class="">
											<a href="<%=basePath%>user/detail?uid=${line.id }" target="_blank">${line.phone }</a>
											<c:if test="${line.real_name !=null}" >
												<a href="<%=basePath%>user/detail?uid=${line.id }" target="_blank">(${line.real_name})</a>
											</c:if>
									</td>
									<td class="" title="${line.gift_name}">
										<c:if test="${fn:length(line.gift_name) > 10}">
											${fn:substring(line.gift_name, 0, 10)}...
										</c:if>
										<c:if test="${fn:length(line.gift_name) <= 10}">
											${line.gift_name}
										</c:if>
									</td>
									<td class=""><fmt:formatNumber value="${line.amount }" type="currency" pattern="￥#,##0.0#"/></td>
									<td class="">
									...
									<td class=""><fmt:formatDate value="${line.start_date }" type="both" pattern="yyyy年MM月dd日" />-<fmt:formatDate value="${line.end_date }" type="both" pattern="yyyy年MM月dd日" /></td>
									<td class=""><fmt:formatDate value="${line.create_time }" type="both" pattern="yyyy年MM月dd日 HH点mm分ss秒" /></td>
									<td class=""><fmt:formatDate value="${line.used_time }" type="both" pattern="yyyy年MM月dd日 HH点mm分ss秒" /></td>
								</tr>
							</c:forEach>
		</tbody>
	</table>
		...
		layui.use(['layer','laypage','element','carousel'], function(){
			...
			laypage.render({ 
				...
				jump:function(obj,first){
					if(!first){
						window.location=path+'/sys/fund/gift/toReceive.action?currentPage='+obj.curr+'&limit=10';
					}
				}
			});
		});

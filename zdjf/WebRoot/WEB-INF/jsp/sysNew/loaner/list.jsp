...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			<a href="javascript:;" onclick="picture_add('新增借款方','../../sys/loaner/detail.action','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 新增借款方</a>
		</span>
	</div>
	<div class="text-c">
			...
			<div class="tabCon">
				<div class="page-container">
					...
					<table class="table table-border table-bordered table-hover table-bg table-sort">
						...
						<tbody>
							<c:forEach var="loaner"  items="${detail.loaner.dataList}">
                                            <tr class="text-c">
                                            	<td>${loaner.account }</td>
                                                <td>${loaner.idcard }</td>
                                                <td>${loaner.name }</td>
                                                <td>${loaner.phone }</td>
                                              	<td>${loaner.age }</td> 
                                                ...
                                                <td>
                                                <c:if test="${loaner.status == 1}">有效</c:if>
                                                <c:if test="${loaner.status != 1}">无效</c:if>
                                                </td>
                                                <td><fmt:formatDate value="${loaner.create_time }" pattern="yyyy年MM月dd日 HH时mm分ss秒" /></td>
                                                <td>
                                                    <div class="am-btn-toolbar">
                                                        <div class="am-btn-group am-btn-group-xs">

                                                           <%--  <c:if test="${!empty sessionScope.auths['loaner.update']}" > 
                                                            </c:if>
                                                            <c:if test="${!empty sessionScope.auths['loaner.delete']}" > 
                                                             </c:if> --%>
                                                             <a title="编辑" href="javascript:;" onclick="member_edit('编辑','../../sys/loaner/detail.action','${loaner.id}','900','700')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
			...
			<div class="tabCon">
				...
				<div class="mt-20">
					<table class="table table-border table-bordered table-hover table-bg table-sort">
						...
						<tbody>
							<c:forEach var="loaner"  items="${detail.comp.dataList}">
                                            <tr>
                                                <td>${loaner.comp_name }</td>
                                                <td>${loaner.comp_code }</td>
                                                <td>${loaner.comp_address }</td>
                                              	<td>${loaner.reg_date }</td> 
                                                <td>${loaner.name }</td> 
                                                <td>${loaner.phone }</td> 
                                                <td>
                                                <c:if test="${loaner.status == 1}">有效</c:if>

...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 会员管理 <span class="c-gray en">&gt;</span> 验证码列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
				...
				<c:forEach var="line"  items="${detail.params.dataList }">
								<tr class="text-c">
									...
									<td class="am-text-middle">
										<%-- <c:if test="${!empty line.uid && line.uid != 0 && !empty sessionScope.auths['user.detail']}"><a href="<%=basePath%>user/detail?uid=${line.uid }" target="_blank">${line.phone }</a></c:if>
									...
									<td class="am-text-middle">${line.code }</td>
		...
		layui.use(['layer','laypage','element','carousel'], function(){
			...
			laypage.render({ 
				...
				jump:function(obj,first){
					if(!first){
						window.location=path+'/sys/sms/smsCode.action?currentPage='+obj.curr+'&limit=10';
					}
				}
			});
		});

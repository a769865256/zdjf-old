...
<body>
	<nav class="breadcrumb">
		...
		财务管理 <span class="c-gray en">&gt;</span> 项目还本付息管理 <a
	...
	<div class="page-container">
		...
		<div class="mt-20">
			<table
				...
				<thead>
					<tr class="text-c">
						<th class="table-type">债权编号</th>
                        <th class="table-type">项目编码</th>
                        <th class="table-type">出借人</th>
                        <th class="table-type">发布时间</th>
                        <th class="table-type">期数</th>
                        <th class="table-type">还款日</th>
                        <th class="table-type">距离到期</th>
                        <th class="table-type">待付本金</th>
                        <th class="table-type">已付本金</th>
                        <th class="table-type">待付利息</th>
                        <th class="table-type">已付利息</th>
                        <th class="table-type">还款状态</th>
				...
				<tbody>
					...
					<c:forEach var="pro" items="${page.dataList }">
						<tr>
							...
							<td>${pro.loaner_name }</td>
							<td>${pro.product_code }</td>
							<td>${pro.lender_balance }</td>
							<td>${pro.issure_time }</td>
							<td>${pro.num }</td>
							<td>${pro.pay_day }</td>
							<td>
								<c:if test="${pro.expire_day<0 }">-</c:if>
								<c:if test="${pro.expire_day>=0 }">${pro.expire_day}</c:if>
							</td>
							<td>${pro.to_pay_amount }</td>
							<td>${pro.payed_amount }</td>
							<td>${pro.to_pay_income }</td>
							<td>${pro.payed_income }</td>
							<td>${pro.status }</td>
						</tr>
					</c:forEach>
				</tbody>
		...
		layui.use(['layer','laypage','element','carousel'], function(){
			...
			laypage.render({  /*参考链接http://www.layui.com/doc/modules/laypage.html*/
				...
				jump:function(obj,first){
					if(!first){
						window.location=path+'/sys/finance/repaymentInterest/toList.action?currentPage='+obj.curr+'&limit=10';
					}
				}
			});
		})

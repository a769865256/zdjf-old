...
${lender.lender_type == 2 ? "营业执照" : "身份证号"}：<u><span style="min-width: 303px; display: inline-block; text-decoration: none; border-bottom: 1px #000 solid;" lang=EN-US>
 ...
 <table class="MsoNormal"Table border=1 cellspacing=0 cellpadding=0
  ...
  padding:0cm 5.4pt 0cm 5.4pt'>
    <p class="MsoNormal" align=left style='text-align:left;'><span
            lang=EN-US >&nbsp;
  <fmt:formatDate value="${product.start_date }" pattern="yyyy年MM月dd日"/>
  </span></p>
   </td>
  ...
  padding:0cm 5.4pt 0cm 5.4pt'>
    <p class="MsoNormal" align=left style='text-align:left;'><span
            lang=EN-US >&nbsp;
   <c:if test="${empty productBuy}">${interest}</c:if>
   <c:if test="${not empty productBuy}">${productBuy.product_interest}</c:if>%
  </span></p>
   </td>
  </tr>
  <tr>
  ...
  padding:0cm 5.4pt 0cm 5.4pt'>
    <p class="MsoNormal" align=left style='text-align:left;'><span
            lang=EN-US >&nbsp; <fmt:formatDate value="${product.start_date}" pattern="yyyy年MM月dd日"/>至<fmt:formatDate value="${product.end_date}" pattern="yyyy年MM月dd日"/></span></p>
   </td>
   <td width=142 valign=top style='width:106.55pt;border-top:none;border-left:
  none;border-bottom:solid black 1.0pt;border-right:solid black 1.0pt;
  padding:0cm 5.4pt 0cm 5.4pt'>
...
'>&nbsp;</span></p>
 ...
 <div class="user">
  ...
  <div class="comp">
   ...
   <p>日期：${currDateStr }<span class="seal">
  <c:if test="${type!=1 }">
   <%--（盖章）--%>
